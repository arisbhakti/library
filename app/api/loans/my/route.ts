...
function parseStatusFilter(value: string | null): LoanStatusFilter {
  if (
    value === "all" ||
    value === "active" ||
    value === "returned" ||
...
export async function GET(request: Request) {
  try {
    ...
    const limit = parsePositiveInteger(searchParams.get("limit"), 3);
    ...
    const url = `${baseUrl.replace(/\/+$/, "")}/loans/my`;
